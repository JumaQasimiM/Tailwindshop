...
const Navbar = () => {
  ...
  return (
    <nav className="flex justify-between items-center bg-slate-900 p-3 z-100 text-white">
      <div className="flex items-center gap-0 text-orange-900 font-semibold cursor-pointer">
        <img src={logo} alt="" className="w-10 h-10" />
        <h2 className="text-2xl font-bold text-pink-500">
          Shop<span className="text-prink-600">.</span>
        </h2>
      </div>
      ...
      <div className="flex gap-3 items-center justify-center md:hidden cursor-pointer">
        <div className="relative">
          ...
          <span className="absolute -top-1 -right-2 bg-orange-500 text-white rounded-full min-w-4 h-4 flex justify-center items-center">
            2
          </span>
        </div>
      ...
      <ul
        className={
          open
            ? "w-[70%] md:hidden fixed top-0 left-0 sm:w-[60%] h-screen bg-slate-400 border-r border-r-slate-900 ease-in-out duration-500 flex flex-col items-center z-10000"
      ...
      >
        ...
        <button className="nav-btn" onClick={handleOpen}>
          {" "}
          <Link to={"/Tailwindshop"}>Sign Up</Link>
        </button>
      </ul>
    </nav>
  );
